...
int ripemd256_low_init(ripemd256_state *context) {
	tc_memset(context, 0, sizeof(ripemd256_state));

	RIPEMD256_init(context->digest);
...
int ripemd256_low_update(ripemd256_state *context, const unsigned char *in, size_t in_len) {
	...
	if (context->blen && ((context->blen + in_len) >= sizeof(context->block))) {
		...
		RIPEMD256_compress(context->digest, (uint32_t *) context->block);
	...
	for (context->blen = 0, i = in_len; i >= sizeof(context->block); i -= sizeof(context->block)) {
		RIPEMD256_compress(context->digest, (uint32_t *) in);
...
int ripemd256_low_final(ripemd256_state *context, unsigned char *out) {
	...
	RIPEMD256_finish(context->digest, context->block, context->mlen, 0);
